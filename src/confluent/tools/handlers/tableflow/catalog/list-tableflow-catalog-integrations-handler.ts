...
export class ListTableFlowCatalogIntegrationsHandler extends TableflowWithKafkaToolHandler {
  async handle(
    ...
    if (error) {
      return this.createResponse(
        `Failed to list Tableflow catalog integrations for ${kafka_cluster_id}: ${JSON.stringify(error)}`,
        true,
      );
    }
    return this.createResponse(
