...
const getConnectorTasksArguments = z.object({
  environmentId: z
    .string()
    .trim()
    .optional()
...
export class GetConnectorTasksHandler extends BaseToolHandler {
  async handle(
    clientManager: ClientManager,
    toolArguments: Record<string, unknown> | undefined,
  ): Promise<CallToolResult> {
    const { clusterId, environmentId, connectorName } =
      getConnectorTasksArguments.parse(toolArguments);
  ...
  getToolConfig(): ToolConfig {
    return {
      ...
      inputSchema: getConnectorTasksArguments.shape,
