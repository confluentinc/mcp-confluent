...
describe("parseCliArgs --oauth", () => {
  ...
  it("should accept stag and prod for --oauth-env", () => {
    expect(
      parseCliArgs(["node", "mcp-confluent", "--oauth", "--oauth-env", "stag"])
        .oauthEnv,
    ).toBe("stag");
    expect(
      parseCliArgs(["node", "mcp-confluent", "--oauth", "--oauth-env", "prod"])
        .oauthEnv,
    ).toBe("prod");
  });

  it("should reject --oauth without --oauth-env", () => {
    expect(() => parseCliArgs(["node", "mcp-confluent", "--oauth"])).toThrow(
      /--oauth-env/,
    );
  });
