...
const getConnectorOffsetsArguments = z.object({
  environmentId: z
    .string()
    .trim()
    .optional()
...
export class GetConnectorOffsetsHandler extends BaseToolHandler {
  async handle(
    clientManager: ClientManager,
    toolArguments: Record<string, unknown> | undefined,
  ): Promise<CallToolResult> {
    const { clusterId, environmentId, connectorName } =
      getConnectorOffsetsArguments.parse(toolArguments);
  ...
  getToolConfig(): ToolConfig {
    return {
      ...
      inputSchema: getConnectorOffsetsArguments.shape,
