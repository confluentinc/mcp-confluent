  ...
  () => {
    ...
    describe(`with a ${ConnectionType.DIRECT} connection`, () => {
      ...
      describe.each(activeTransports)("via %s transport", (transport) => {
        ...
        it("should register a new subject via the tool", async () => {
          const subject = uniqueName(`create-${transport}`);
          ...
          // SR's listing is eventually consistent; poll until the tool-created subject is visible
          await expect
    ...
    describe(
      ...
      () => {
        ...
        // without direct creds skips cleanly instead of crashing in afterAll cleanup
        if (
          skipIfDisabled(
            handler,
