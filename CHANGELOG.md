...
- **`consume-messages` tool: breaking shape changes alongside the new controls above.**
  ...
  - **Schema Registry decoding now defaults on; opt out per side with `disableSchemaRegistry: true`.** The old `useSchemaRegistry: true` opt-in is replaced with `disableSchemaRegistry: false` (the default) on `valueFormat` / `keyFormat`. When the connection has SR reachable (a `schema_registry` block, or any OAuth connection), messages auto-decode via the registered AVRO / JSON / PROTOBUF schema; topics without a subject and SR client failures fall back to raw silently. Closes #475.
