...
const pauseConnectorArguments = z.object({
  environmentId: z
    .string()
    .optional()
    .describe(
  ...
  connectorName: z
    .string()
    .nonempty()
    .describe("The name of the connector to pause."),
});

export class PauseConnectorHandler extends BaseToolHandler {
  async handle(
    clientManager: ClientManager,
    toolArguments: Record<string, unknown> | undefined,
  ): Promise<CallToolResult> {
    const { clusterId, environmentId, connectorName } =
      pauseConnectorArguments.parse(toolArguments);
  ...
  getToolConfig(): ToolConfig {
    return {
      ...
      inputSchema: pauseConnectorArguments.shape,
